    ...
    match msg {
        ...
        WM_CTLCOLORSTATIC => {
            ...
            if id == IDC_STATUS_TEXT {
                ...
                return LRESULT((*st).br_sect.0 as isize);
            }
            SetTextColor(
                hdc,
                COLORREF(if id == IDC_VERSION { C_HDR } else { C_LABEL }),
            );
            let br = if id == IDC_VERSION {
                (*st).br_win
            } else {
                (*st).br_sect
            };
...
unsafe fn on_create(hwnd: HWND) {
    ...
    let state = Box::new(WndState {
        ...
        br_green: CreateSolidBrush(COLORREF(C_SECT_BG)),
        br_red: CreateSolidBrush(COLORREF(C_SECT_BG)),
        cards: Vec::new(),
        focused_edit: 0,
    });
...
unsafe fn on_app_connected(hwnd: HWND, wp: WPARAM) -> LRESULT {
    ...
    stmut(hwnd).connected = connected;
    if connected {
